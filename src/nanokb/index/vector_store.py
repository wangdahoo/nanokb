...
class VectorStore:
    ...
    def index_nodes(
        ...
        *,
        ...
        Args:
            graph: 待索引的子图（节点带 description / source_file）。
            llm: embedding 客户端；当 ``embed_fn`` 为 None 时用于直接 embed。
            embed_fn: 可选的批量 embed 函数（方案 §4.6 round 2 Severe #1）。
                提供时一次性接收**全部** description 文本，返回等长向量列表
                （通常为 ``EmbeddingCache.embed_batch``）；为 None 时走 ``llm.embed``
                零回归路径。embed 切批与 upsert 切批在此解耦——本方法对返回的
                embeddings 始终按 ``EMBED_BATCH_SIZE`` 分批串行 ``col.upsert``。
            on_progress: 可选的 upsert 进度回调 ``(done, total)``。

        Raises:
            RuntimeError: ``embed_fn`` / ``llm.embed`` 返回的向量数 != items 数
                （方案 §4.6 round 2 Severe #1 防御性契约校验，禁止静默截断）。

        关键不变式（round 3 Opt#5）：无论向量来自 cache 命中还是新 embed，
        ``index_nodes`` 对 embeddings **始终全量 upsert**——cache 只省 embed HTTP
        调用，向量必须进 ChromaDB 才能被 ``search`` 召回。
        """
        ...
        # ★ round 2 Severe #1 修复：一次性把全部 texts 传给 embed_fn，不做外层 64 切分
        # （batch 归属单一化）。embed_fn 默认 None 时走 llm.embed 零回归路径。
        if embed_fn is None:
        ...
        if len(embeddings) != len(items):
            logger.error(
                "embed returned %d vectors for %d items; aborting index_nodes",
                len(embeddings),
                len(items),
                extra={"stage": "vector-store"},
            )
            raise RuntimeError(
                f"embed length mismatch: {len(embeddings)} != {len(items)}"
            )

        # 串行 upsert，按 EMBED_BATCH_SIZE 分批（upsert 批大小，与 embed 切批解耦）。
        # round 3 Opt#5：无论向量来自 cache 命中还是新 embed，均全量 upsert 进 ChromaDB。
        for start in range(0, len(items), EMBED_BATCH_SIZE):
            ...
            col.upsert(
                ids=[node_id for node_id, _, _, _ in batch_items],
                embeddings=[list(map(float, e)) for e in batch_vecs],
                documents=[desc for _, _, _, desc in batch_items],
                metadatas=[
                    {"source_file": sf, "node": node} for _, sf, node, _ in batch_items
                ],
            )
            if on_progress:
